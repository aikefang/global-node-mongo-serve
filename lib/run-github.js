...
const docCategoryModel = require('../app/models/doc/doc-category')
...
module.exports = async ({list, errorCallback = () => {}, successCallback = () => {}}) => {
  for (let i = 0; i < list.length; i++) {
    ...
    if (hasData) {
      ...
      const iconData = await docCategoryModel.findOne({
        title: firstPath
      })
      if (!iconData) {
        common.log('find-icon-error', {
          path: noMdPath
        })
      }
      otherData.icon = firstPath
    }
    // 检查是否为新数据 End
    ...
    // 更新提交记录
    const docCommitData = await docCommitModel.findOneAndUpdate(
      {
        path: noMdPath
      },
      {
